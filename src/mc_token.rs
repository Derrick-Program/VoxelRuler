...
const MS_CLIENT_ID: &str = "ebd68e7a-2003-487d-bfa6-14807af049c9";
const MS_AUTH_URL: &str = "https://login.microsoftonline.com/consumers/oauth2/v2.0/authorize";
const MS_TOKEN_URL: &str = "https://login.microsoftonline.com/consumers/oauth2/v2.0/token";
const REDIRECT_URI: &str = "http://127.0.0.1:8114/redirect";
...
fn build_http_client() -> anyhow::Result<oauth2::reqwest::Client> {
    Ok(oauth2::reqwest::ClientBuilder::new()
        .redirect(reqwest::redirect::Policy::none())
        .build()?)
}

async fn mint_and_save_mc_session(
...
async fn await_oauth_redirect() -> anyhow::Result<(AuthorizationCode, CsrfToken)> {
    ...
    let code = AuthorizationCode::new(code_val.into_owned());
    let state = CsrfToken::new(state_val.into_owned());
...
pub async fn refresh_minecraft_token(saved_refresh_token: &str) -> anyhow::Result<String> {
    let client = BasicClient::new(ClientId::new(MS_CLIENT_ID.to_string()))
    ...
    let token_response = client
        .exchange_refresh_token(&RefreshToken::new(saved_refresh_token.to_string()))
        .request_async(&http_client)
        .await?;

    let ms_access_token = token_response.access_token().secret().clone();
    let ms_refresh_token = token_response
        .refresh_token()
        .map_or_else(|| saved_refresh_token.to_string(), |rt| rt.secret().clone());
    ...
    F: FnOnce(String) + Send + 'static,
{
    ...
    on_url_generated(authorize_url.to_string());
    let _ = open::that(authorize_url.as_str());

    let (code, state) = await_oauth_redirect().await?;
    ...
    let ms_access_token = token.access_token().secret().clone();
