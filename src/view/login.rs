...
fn handle_login_success(ui_weak: slint::Weak<MainApp>, username: String, is_premium: bool, avatar_path: Option<std::path::PathBuf>) {
    let authenticator_text = if is_premium { "Microsoft (Premium)" } else { "Microsoft (Unpaid)" };
    let _ = slint::invoke_from_event_loop(move || {
        if let Some(ui) = ui_weak.upgrade() {
            let avatar_img = avatar_path
                .and_then(|p| slint::Image::load_from_path(&p).ok())
...
fn handle_refresh_success(ui_weak: slint::Weak<MainApp>, idx: usize, status_text: &str, avatar_path: Option<std::path::PathBuf>) {
