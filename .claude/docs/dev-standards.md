...
- 預設不寫註解，包含 `///` / `//!` doc comment。命名與程式結構應自解釋。
- 僅在以下情況允許加註解（且只能極簡短一行）：隱藏限制、不明顯的不變量、針對特定 bug 的 workaround、`.unwrap()` 安全性證明等無法從程式碼本身看出的資訊。
- 不寫「做了什麼」的說明性註解，不寫任務/修復紀錄型註解。

### 錯誤處理

- 使用 `anyhow::Result` 作為函式回傳型別
- 對使用者可見的錯誤，透過 Slint callback 傳至 UI 顯示
- 不允許直接 `.unwrap()`（除非確保不會 panic，且需以極簡短註解說明原因，參見上方註解規範）
