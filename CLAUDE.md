...
- 預設不寫程式碼註解。程式碼本身（good naming）應該足以說明「做什麼」。
- 僅在無法從程式碼本身看出的情境才加註解：隱藏限制、不明顯的不變量、針對特定 bug 的 workaround、會讓人意外的行為。
- 不寫「這是做什麼」的說明性註解，也不寫任務/修復紀錄型註解（那些屬於 commit message 或 PR 說明）。
